...
async function resolveTools() {
    ...
    if (!cfg.mcpServers?.length)
        return { tools: ALL_TOOLS, closeAll: () => { } };
...
async function resolveProviderAndModel(opts) {
    ...
    const preset = custom
        ? { id: custom.id, label: custom.label, kind: "openai-compatible", baseUrl: custom.baseUrl, envKey: custom.envKey, defaultModel: custom.defaultModel }
    ...
    const provider = buildProvider({ id: preset.id, apiKey: apiKey ?? "not-configured", baseUrl: override?.baseUrl ?? preset.baseUrl, model }, preset.kind);
...
program
    ...
    try {
        await agent.send(promptParts.join(" "), {
            ...
            onToolResult: (name, result, isError) => console.log(isError ? chalk.red(`✗ ${name}: ${result}`) : chalk.gray(`✓ ${name}`)),
        });
    }
    finally {
        closeAll();
...
const providersCmd = program
    ...
    .action(async () => {
    const cfg = await loadConfig();
    const isConfigured = (p) => Boolean(process.env[p.envKey] || cfg.providerOverrides?.[p.id]?.apiKey);
...
providersCmd
    ...
    const isConfigured = (id, envKey) => Boolean(process.env[envKey] || cfg.providerOverrides[id]?.apiKey);
    const setupProviders = async (providers) => {
        let count = 0;
