...
export const editFileTool: Tool = {
  ...
  definition: {
    name: "edit_file",
    description:
      "Replace an exact, unique substring within a file. old_str must match the file's " +
  ...
  async run(input, ctx) {
    ...
    if (occurrences === 0) return `Error: old_str not found in ${filePath}`;
    if (occurrences > 1) return `Error: old_str matches ${occurrences} times in ${filePath}; must be unique.`;
