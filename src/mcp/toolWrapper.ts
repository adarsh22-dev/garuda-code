 ...
 */
export async function loadMcpTools(configs: McpServerConfig[]): Promise<LoadedMcpServers> {
  ...
  for (const cfg of configs) {
    try {
      ...
      console.error(`[garuda] Failed to load MCP server "${cfg.name}": ${(err as Error).message}`);
    }
  }
