...
async function runCommandSafe(command, cwd, timeout = 30000) {
    try {
...
export const lintTool = {
    ...
    async run(input, ctx) {
        ...
        if (!cmds)
            return `No linter configured for project type: ${projectType}. Detected: ${target}`;
...
export const typecheckTool = {
    ...
    async run(input, ctx) {
        ...
        if (!cmd)
            return `No type checker configured for project type: ${projectType}`;
        return `Project type: ${projectType}\nCommand: ${cmd}\n\n${await runCommandSafe(cmd, ctx.cwd)}`;
    },
};
